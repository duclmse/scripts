...
def parse_resource_string(resource_str: str) -> tuple[str, str | None]:
    """
...
def parse_pod_status(pod_data: dict) -> dict:
    """
    ...
    if container_statuses:
        # Count restarts
        result['restarts'] = sum(c.get('restartCount', 0)
                                 for c in container_statuses)

        # Check if all containers are ready
        result['ready'] = all(c.get('ready', False)
                              for c in container_statuses)
...
def parse_resource_requirements(resources: dict) -> dict[str, dict[str, float]]:
    """
