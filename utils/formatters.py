...
def format_table(data: list[list[str]], headers: list[str] | None = None,
                 align: str = 'left') -> str:
    """
...
def format_yaml(data, indent: int = 2) -> str:
    """
    Format data as YAML
...
def format_json(data, indent: int = 2, compact: bool = False) -> str:
    """
    Format data as JSON
...
def format_list(items: list[str], separator: str = ', ', max_items: int | None = None) -> str:
    """
...
def format_resource_list(resources: list[dict], columns: list[str]) -> str:
    """
