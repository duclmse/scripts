...
class ARM64Assembler:
    ...
    def _handle_directive(self, line: str):
        ...
        if directive == '.global' or directive == '.globl':
            ...
            # Parse section name
            section = args.strip().strip('"')
            if '__TEXT' in section or '__text' in section:
                self.current_section = 'text'
            else:
                self.current_section = 'data'
            ...
            # Align to power of 2
            alignment = int(args.strip())
            align_bytes = 1 << alignment
            while self.address % align_bytes != 0:
                self.address += 4
                if self.current_section == 'text':
                    self.machine_code.append(0xD503201F)  # NOP

        elif directive == '.balign' or directive == '.p2align':
            alignment = int(args.strip())
        ...
        elif directive in ['.quad', '.8byte']:
            # 64-bit data
            value = self.parse_imm(args.strip())
            self.data_section.append(struct.pack(
                '<Q', value & 0xFFFFFFFFFFFFFFFF))

        elif directive in ['.word', '.long', '.4byte']:
            # 32-bit data
            value = self.parse_imm(args.strip())
            self.data_section.append(struct.pack('<I', value & 0xFFFFFFFF))

        elif directive in ['.short', '.2byte']:
            # 16-bit data
            value = self.parse_imm(args.strip())
            self.data_section.append(struct.pack('<H', value & 0xFFFF))

        elif directive in ['.byte', '.1byte']:
            # 8-bit data
            value = self.parse_imm(args.strip())
            self.data_section.append(struct.pack('<B', value & 0xFF))

        elif directive == '.ascii':
            # ASCII string (no null terminator)
            string = args.strip().strip('"')
            self.data_section.append(string.encode('ascii'))

        elif directive == '.asciz' or directive == '.string':
            # ASCII string with null terminator
            string = args.strip().strip('"')
            self.data_section.append(string.encode('ascii') + b'\x00')

        elif directive == '.space' or directive == '.skip':
            # Reserve space
            size = int(args.strip())
            self.data_section.append(b'\x00' * size)

        elif directive in ['.type', '.size', '.ident', '.file']:
            # Ignore these directives (metadata)
            pass

        else:
            # Unknown directive - just ignore it
            pass
    ...
    def assemble(self, source: str, base_address: int = 0x100000000) -> List[int]:
        ...
        for line in lines:
            try:
                ...
                print(f"Error on line '{line.strip()}': {e}", file=sys.stderr)
                raise
...
"""
        ...
        print("  .global/.globl  - Mark symbol as global")
        print("  .text/.data     - Switch sections")
        print("  .align/.balign  - Alignment")
        print("  .quad/.word     - Data definitions")
        print("  .ascii/.asciz   - String data")
        print("  .space/.skip    - Reserve space")
