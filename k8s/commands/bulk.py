...
@Command.register("b", aliases=["bulk"], help="Bulk operations on resources", args=[
    arg("action", help="Bulk action to perform",
        choices=["delete", "restart", "scale", "label", "annotate", "exec", "logs", "describe", "patch"]),
...
class BulkCommand:
    ...
    def get_resources(self, args) -> list[dict]:
    ...
    def bulk_restart(self, name: str, args):
        ...
        match args.type:
            case "pod":
                ...
                self.kube.run([
                    "delete", "pod", name, "-n", self.kube.namespace])
            case "deployment":
                # Rollout restart
                self.kube.run([
