...
@Command.register("compare", help="Compare resources across contexts/namespaces", args=[
    arg("resource_type", help="Resource type to compare (deployment, svc, etc)"),
    ...
    # Output options
    arg("--format", choices=["table", "yaml", "json", "diff"],
...
class CompareCommand:
    ...
    def __init__(self, kube):
        ...
        self.comparisons = []
        self.differences = []
        self._resources_by_target: dict[str, dict] = {}  # label → {name → cleaned resource}
    ...
    def execute(self, args):
        ...
        # Compare resources (also populates self._resources_by_target)
        ...
        if args.output_file:
            self.save_to_file(args.output_file, args.format)
    ...
    def fetch_resources(self, target: dict[str, str], args) -> dict[str, dict]:
        ...
        try:
            ...
            if 'items' in data:
                resources = {item['metadata']['name']: item for item in data['items']}
            else:
    ...
    def compare_resource_versions(self, versions: dict[str, dict], args) -> list[dict]:
        ...
        # Clean resources for comparison and cache for later diff rendering
        cleaned_versions = {}
        for label, resource in versions.items():
            cleaned = self.clean_resource(resource, args)
            cleaned_versions[label] = cleaned
            self._resources_by_target.setdefault(label, {})[
                resource['metadata']['name']] = cleaned
    ...
    def _get_stored_resource(self, label: str, name: str) -> dict | None:
        """Return the cleaned resource dict cached during compare_resource_versions."""
        return self._resources_by_target.get(label, {}).get(name)
    ...
    def find_differences(self, res1: dict, res2: dict, label1: str, label2: str, args) -> list[dict]:
    ...
    def get_field_value(self, obj: dict, path: str):
    ...
    def deep_compare(self, obj1, obj2, label1: str, label2: str, path: str = "") -> list[dict]:
    ...
    def display_table(self, args):
        ...
        for comp in self.comparisons:
            if not comp['differences'] and not args.show_all:
                continue
    ...
    def display_diff(self, args, targets: list[dict]):
        """Display unified diff of YAML representations between two targets."""
        if len(targets) != 2:
            Logger.error("Diff output requires exactly 2 targets")
            return

        label1, label2 = targets[0]['label'], targets[1]['label']
        context_lines = getattr(args, 'diff_context', 3)
        use_color = getattr(args, 'color_diff', True) and Colors.RED != ''
        ...
        for comp in self.comparisons:
            ...
            found_any = True
            name = comp['name']
            print(f"\n{Colors.BOLD}--- {label1}/{name}{Colors.RESET}")
            print(f"{Colors.BOLD}+++ {label2}/{name}{Colors.RESET}")
            print("-" * 60)

            # Re-fetch cleaned resource YAML from both targets
            res1 = self._get_stored_resource(label1, name)
            res2 = self._get_stored_resource(label2, name)

            lines1 = yaml.dump(res1, default_flow_style=False).splitlines(keepends=True) if res1 else []
            lines2 = yaml.dump(res2, default_flow_style=False).splitlines(keepends=True) if res2 else []

            diff = list(difflib.unified_diff(
                lines1, lines2,
                fromfile=f"{label1}/{name}",
                tofile=f"{label2}/{name}",
                n=context_lines,
            ))

            if not diff:
                print("  (no text differences after cleaning)")
                continue
