...
from k8s.config.config import KubeConfig
from core.decorators import Command, arg
from core.logger import Logger


@Command.register("ct", aliases=["ctx", "context"], help="Manage contexts", args=[
    arg("context_name", nargs="?", help="Context name"),
    arg("action", choices=["ls", "list", "use", "cr", "current", "bm", "bookmark"],
...
class ContextCommand:
    ...
    def bookmark_add(self, args):
        ...
        ctx = args.context_name if hasattr(args, 'context_name') \
            and args.context_name else self.get_current_context()
        ns = args.bookmark_namespace if hasattr(args, 'bookmark_namespace') \
            and args.bookmark_namespace else self.kube.namespace
