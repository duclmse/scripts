...
@Command.register("deps", help="Show resource dependencies", args=[
    arg("resource_type", nargs="?", help="Resource type (deployment, service, pod, etc)"),
    arg("resource_name", nargs="?", help="Specific resource name"),

    # Display options
    arg("-o", "--output", choices=["tree", "graph", "json", "dot", "mermaid"], default="tree", help="Output format"),
    ...
    arg("--include-external", action="store_true", help="Include external dependencies (configmaps, secrets)"),

    # Analysis options
    arg("--find-cycles", action="store_true", help="Find circular dependencies"),
    ...
    arg("--format-output", action="store_true", help="Format output (for dot/mermaid)"),
])
class DepsCommand:
    ...
    def fetch_all_resources(self, args):
        ...
        resource_types = [
            "pods", "deployments", "replicasets", "statefulsets", "daemonsets", "services", "ingresses",
